...
bool TOrderedLinkList<Type>::Insert( Type value )
{
	...
	CNode  *min=new CNode(0);
	CNode  *max=new CNode(9999999);
	if(TOrderedLinkList<Type>::Search(value))//判断链表中是否已含有待插入的值 
	{
		cout<<value<<"已在链表中！"<<endl;
	...
	else
	{
		...
		if(TLinkList<Type>::m_pHead==NULL)//在表头和表尾插入最小值0和最大值 9999999
		{
			min->m_pPre=NULL;
			max->m_pNext=NULL;
			min->m_pNext=max;
			max->m_pPre=min;
			TLinkList<Type>::m_pHead=min;
		}
		else
		{
			min->m_pPre=NULL;
			max->m_pNext=NULL;
			min->m_pNext=TLinkList<Type>::m_pHead;
			TLinkList<Type>::m_pHead->m_pPre=min;
			p1=TLinkList<Type>::m_pHead;
		...
		p1=TLinkList<Type>::m_pHead;
		while(p1->m_pNext!=NULL)//删除插入的最大值 
		{
			p1=p1->m_pNext;
		}
		(p1->m_pPre)->m_pNext=NULL;
		p1->m_pPre=NULL;
		p1=TLinkList<Type>::m_pHead;
		(p1->m_pNext)->m_pPre=NULL;//删除插入的最小值 
		TLinkList<Type>::m_pHead=p1->m_pNext;
		p1->m_pNext=NULL;
		delete min;
		delete max;
...
void TOrderedLinkList<Type>::Print( )
{
	...
	//遍历链表以读出每一个节点的值并在终端上显示。
	for ( CNode  *p = TLinkList<Type>::m_pHead; p != NULL; p = p->m_pNext )
	{
		cout << " " << p->m_value << ", ";      //显示节点值
	}
	cout << endl;								//结束输出流
