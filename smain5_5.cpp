...
bool TOrderedLinkList<Type>::Insert( Type value )
{
	...
	if (TLinkList<Type>::m_pHead == NULL)					//表头为空时插入为第一个节点
	{
		pTemp->m_pNext = NULL;				//1. 新节点的后继为NULL
		pTemp->m_pPre = NULL;				//2. 新节点的前驱为NULL
		TLinkList<Type>::m_pHead = pTemp;					//3. 头指针指向新节点
	}
	else			//当表头不为空时，首先要寻找插入位置，然后才能插入节点		
	{
		if (value<TLinkList<Type>::m_pHead->m_value)			//在表的头节点前插入
		{
			pTemp->m_pNext = TLinkList<Type>::m_pHead;		// 1. 将原头节点设置为新头节点的后继
			pTemp->m_pPre = NULL;			// 2. 将新头节点的前驱设置为NULL
			TLinkList<Type>::m_pHead->m_pPre = pTemp;		// 3. 将新头节点设置为原头节点的前驱
			TLinkList<Type>::m_pHead = pTemp;				// 4. 将新头节点设置为新的表头。
		}
		else								//不是在头节点前插入 
		{	//让p2指向p1的前驱(主要考虑链表尾部的插入)
			for ( p2=TLinkList<Type>::m_pHead,p1=TLinkList<Type>::m_pHead->m_pNext; p1!=NULL; p2=p1,p1=p1->m_pNext )
			{
